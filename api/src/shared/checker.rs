...
pub fn is_neo_address(string: &str) -> bool {
    string.chars().count() == 34
        ...
        && string.chars().all(|c| ALPHABET.contains(&(c as u8)))
}

#[allow(dead_code)]
pub fn is_neo_script_hash(string: &str) -> bool {
    string.chars().count() == 42
...
fn test_is_neo_script_hash() {
    assert!(is_neo_script_hash(
        "0x6250481ec87ae2052f90ec7cb46d757b8db1c447"
    ));
    assert!(!is_neo_script_hash(
        "0x6250481ec/7ae2052f90ec7cb46#757b8db1c447"
    ));
    ...
    assert!(!is_neo_script_hash(
        "0x3184522d88fe2a5e0ac35324d119e6e82b3df7d7c95c927c247155467d49e8ba"
    ));
}

#[test]
fn test_is_neo_txid_hash() {
    assert!(is_neo_txid_hash(
        "0x3184522d88fe2a5e0ac35324d119e6e82b3df7d7c95c927c247155467d49e8ba"
    ));
    assert!(!is_neo_txid_hash(
        "0x31*4522d88fe2a5e0ac35324d119e6e82b3df7d7c95@927c247155467d49e8ba"
    ));
